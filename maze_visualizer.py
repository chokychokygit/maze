...
def load_config_from_json(filename='config.json'):
    """
    Load configuration from JSON file in the same directory as the script
    """
    ...
    try:
        if os.path.exists(config_path):
            ...
            print(f"⚠️ Configuration file {filename} not found in {script_dir}")
...
def get_default_config():
    ...
    return {
        ...
        "debug_settings": {
            ...
            "show_boundary_info": True,
            "show_sensor_readings": True
        }
    }

def save_exploration_data_to_json(graph_mapper, filename_prefix='exploration_data'):
    """
    Save exploration data to JSON file for later analysis
    """
    script_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    exploration_data = {
        ...
        "current_direction": graph_mapper.currentDirection,
        "total_nodes": len(graph_mapper.nodes)
    }
    
    # Convert nodes to JSON format
    for node_id, node in graph_mapper.nodes.items():
        exploration_data["nodes"][node_id] = {
            "id": node.id,
            "position": node.position,
            "walls": node.walls,
            "sensor_readings": node.sensorReadings,
            "explored_directions": list(node.exploredDirections),
            "unexplored_exits": node.unexploredExits,
            "out_of_bounds_exits": node.outOfBoundsExits,
    ...
    try:
        ...
        print(f"💾 Exploration data saved to: {filepath}")
        return filepath
    except Exception as e:
...
if __name__ == '__main__':
    # Check for command line arguments for custom JSON file
    config_filename = 'config.json'
    if len(sys.argv) > 1:
        config_filename = sys.argv[1]
        print(f"🔧 Using custom configuration file: {config_filename}")
    
    # Load configuration from JSON file
    config = load_config_from_json(config_filename)
    ...
    # Initialize components with configuration from JSON
    tof_handler = ToFSensorHandler()
    # Use wall threshold from config
    tof_handler.WALL_THRESHOLD = config['robot_settings']['wall_threshold']
    
    # Use map boundaries from config
    boundaries = config['map_boundaries']
    ...
    try:
        ...
        gimbal_speed = config['robot_settings']['gimbal_speed']
        movement_speed = config['robot_settings']['movement_speed']
        scan_delay = config['exploration_settings']['scan_delay']
        max_nodes = config['robot_settings']['max_nodes']
        
        ep_gimbal.recenter(pitch_speed=gimbal_speed, yaw_speed=gimbal_speed).wait_for_completed()
        ep_gimbal.moveto(pitch=0, yaw=0, pitch_speed=movement_speed, yaw_speed=movement_speed).wait_for_completed()
        ...
        print(f"   ⏱️ Scan Delay: {scan_delay}s")
        print(f"   🗺️ Max Nodes: {max_nodes}")
        ...
        explore_autonomously_with_absolute_directions(ep_gimbal, ep_chassis, ep_sensor, tof_handler, 
                        graph_mapper, movement_controller, attitude_handler, max_nodes=max_nodes)
        ...
        try:
            # Save exploration data to JSON before cleanup
            if 'graph_mapper' in locals():
                save_exploration_data_to_json(graph_mapper)
